...
unsigned short mt_flashReadID(void)
{
	...
	hal_spi2CSDrive(0); 			    
	hal_spi2ReadWriteByte(0x90);//向芯片发送0x90指令    
	hal_spi2ReadWriteByte(0x00);//地址 	    
	hal_spi2ReadWriteByte(0x00);//地址 	    
	hal_spi2ReadWriteByte(0x00);//地址 
	
	Temp|=hal_spi2ReadWriteByte(0xFF)<<8;  //返回的ID
...
void mt_flashWritePage(unsigned char * pBuffer,unsigned int WriteAddr,unsigned short NumByteToWrite)
{
	...
	wAddr = WriteAddr;
	num = NumByteToWrite;
	mt_flashWriteEnable();                  //写操作使能

	hal_spi2CSDrive(0);                              //使能器件   
	hal_spi2ReadWriteByte(0x02);      //发送写页命令   
...
void mt_flashWrite(unsigned int addr,unsigned char *p,unsigned char lon)
{
	...
	mt_flashWrite_handle(p,addr,lon);
